...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc('buy_curve_tokens', {
      p_user_id: user.id,
    ...
    if (error) {
      ...
      if (mapped.status === 500) {
        console.error('[api/curve/buy] buy_curve_tokens error:', error)
      }
    ...
    if (!result) {
      console.error('[api/curve/buy] buy_curve_tokens returned no data')
      return NextResponse.json({ error: 'Failed to buy curve tokens' }, { status: 500 })
    }
    ...
    console.error('[api/curve/buy] unexpected error:', err)
