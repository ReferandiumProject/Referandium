...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const { data, error } = await supabaseAdmin.rpc('admin_delete_startup', {
    ...
    if (error) {
      return handleRpcError('admin_delete_startup', error)
    }

    const result = Array.isArray(data) ? data[0] : data
    return NextResponse.json(result || { ok: true })
  } catch (err: any) {
    console.error('[api/admin/startups/delete] unexpected error:', err)
